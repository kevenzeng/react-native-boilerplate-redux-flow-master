...
class AddOldItemsForm extends Component {
    ...
    render() {
        const { reset, handleSubmit, itemNames, roomNames } = this.props;

        return (
            <View style={{ height: height * 0.5 }}>
                <View style={{ flex: 1, justifyContent: "center" }}>
                    <Field name="olditem_sn" label="序列号" component={renderSimpleInput} validate={[required]}/>
                    <Field name="olditem_name" label="备件名称" component={renderSimpleSBSelect} validate={[required]}>
                    ...
                    <Field name="olditem_num" label="数      量" component={renderSimpleInput} validate={[required, postInt]}/>
                    <Field name="olditem_kufang" label="存放库房" component={renderSimpleSBSelect} validate={[required]}>
											<Picker.Item label={"请选择"} value={"请选择"} key={0} />
											{roomNames.map((value, key) => {
													return <Picker.Item label={`${value.roomname}`} value={value.roomname} key={key} />;
											})}
                    </Field>
                    <View
                        style={{ flexDirection: "row", justifyContent: "space-around", width: "80%", alignSelf: "center", marginTop: px2dp(30) }}>
