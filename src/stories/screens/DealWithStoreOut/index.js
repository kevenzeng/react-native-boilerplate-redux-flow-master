...
export interface Props {
    navigation: any;
    DealWithStoreOutForm: any
}

export interface State {
}

class DealWithStoreOutPage extends React.Component<Props, State> {
    ...
    currentUserReadOnlyControl() {
        ...
        const { operator } = this.props.storeOutInfo; // 既有单的逻辑
        return _.isEqual(userName, operator);
    }

    saveButtonControl() {
        const { status } = this.props.storeOutInfo;
                ...
                <View style={flatList}>
                    ...
                    <View style={flatListItem}>
                        <Button full transparent large style={flatListBtn}
                            ...
                            <Text style={ flatListItemText }>出库申请信息</Text>
                        </Button>
                    </View>
                    ...
                    <View style={flatListItem}>
                        <Button full transparent large style={flatListBtn}
                            ...
                            <Text style={flatListItemText}>出库备件明细</Text>
                        </Button>
                    </View>
                    {renderIf(!this.isNewReceipt() && !_.isNull(this.props.storeOutInfo.applyAduitStatus))(
                        <View style={flatListItemWidth}>
                            ...
                            <View style={flatListItem}>
                                <Button full transparent large style={flatListBtn}
                                    ...
                                    <Text style={flatListItemText}>出库申请审批信息</Text>
                                </Button>
                            </View>
                        </View>
                    )}
                    {renderIf(!this.isNewReceipt() && !_.isNull(this.props.storeOutInfo.checkStatus))(
                        <View style={flatListItemWidth}>
                            <View style={flatListItemFooter}>{}</View>
                            <View style={flatListItem}>
                                <Button full transparent large style={flatListBtn}
                                    ...
                                    <Text style={flatListItemText}>出库复核信息</Text>
                                </Button>
                            </View>
                        </View>
                    )}
                    {renderIf(!this.isNewReceipt() && !_.isNull(this.props.storeOutInfo.checkAduitStatus))(
                        <View style={flatListItemWidth}>
                            <View style={flatListItemFooter}>{}</View>
                            <View style={flatListItem}>
                                <Button full transparent large style={flatListBtn}
                                    ...
                                    <Text style={flatListItemText}>出库复核审批信息</Text>
                                </Button>
                            </View>
                        </View>
                    )}
                    ...
                    <View style={msgListContainer}>
                        <View style={{ flex: 6 }}>
                            <ScrollView ref={(scroll) => this._scroll = scroll}>
                                {this.props.DealWithStoreOutForm}
                            </ScrollView>
                        </View>
                        <View style={footBtnContainer}>
                            ...
                            <View style={footBtn}>
                                <Button full info
                                        onPress={() => this.props.navigation.goBack()}><Text>返回出库管理</Text></Button>
                            </View>
                        </View>
                    </View>
                </View>
...
export default DealWithStoreOutPage;
