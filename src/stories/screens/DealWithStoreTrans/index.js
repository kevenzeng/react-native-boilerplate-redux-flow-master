...
export interface Props {
    navigation: any;
    DealWithStoreTransForm: any
}

export interface State {
}

class DealWithStoreTransPage extends React.Component<Props, State> {
    ...
    currentUserReadOnlyControl() {
        ...
        const { operator } = this.props.storeTransInfo; // 既有单的逻辑
        return _.isEqual(userName, operator);
    }

    saveButtonControl() {
        const { status } = this.props.storeTransInfo;
        if (this.isNewReceipt()) {
            return true;
        } else {
            return this.currentUserReadOnlyControl() &&
                _.includes([ "01" ], status) &&
                ...
                <View style={flatList}>
                    ...
                    <View style={flatListItem}>
                        <Button full transparent large style={flatListBtn}
                            ...
                            <Text style={ flatListItemText }>移库申请信息</Text>
                        </Button>
                    </View>
                    ...
                    <View style={flatListItem}>
                        <Button full transparent large style={flatListBtn}
                            ...
                            <Text style={flatListItemText}>移库备件明细</Text>
                        </Button>
                    </View>
                    {renderIf(!this.isNewReceipt() && !_.isNull(this.props.storeTransInfo.applyAduitStatus))(
                        <View style={flatListItemWidth}>
                            ...
                            <View style={flatListItem}>
                                <Button full transparent large style={flatListBtn}
                                    ...
                                    <Text style={flatListItemText}>移库申请审批信息</Text>
                                </Button>
                            </View>
                        </View>
                    )}
                            ...
                            <View style={flatListItem}>
                                <Button full transparent large style={flatListBtn}
                                        onPress={() => {
                                            ...
                                            console.log("currentStatus", this.props.storeTransInfo.status);
                                        }}>
                    ...
                    <View style={msgListContainer}>
                        <View style={{ flex: 6 }}>
                            <ScrollView ref={(scroll) => this._scroll = scroll}>
                                {this.props.DealWithStoreTransForm}
                            </ScrollView>
                        </View>
                        <View style={footBtnContainer}>
                            ...
                            <View style={footBtn}>
                                <Button full info
                                        onPress={() => this.props.navigation.goBack()}><Text>返回移库管理</Text></Button>
                            </View>
                        </View>
                    </View>
                </View>
...
export default DealWithStoreTransPage;
